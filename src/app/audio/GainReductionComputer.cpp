...
void GainReductionComputer::setRatio(const float ratio) { slope = 1.0f / ratio - 1.0f; }

inline const float GainReductionComputer::applyCharacteristicToOverShoot(
    const float overShootInDecibels) {
    if (overShootInDecibels <= -kneeHalf)
        return 0.0f;
    else if (overShootInDecibels > -kneeHalf && overShootInDecibels <= kneeHalf)
        return 0.5f * slope * (overShootInDecibels + kneeHalf) *
               (overShootInDecibels + kneeHalf) / knee;
    else
        return slope * overShootInDecibels;
}

void GainReductionComputer::computeGainInDecibelsFromSidechainSignal(
    const float* sideChainSignal, float* destination, const int numSamples) {
    ...
    for (int i = 0; i < numSamples; ++i) {
        ...
        if (state < maxGainReduction) maxGainReduction = state;
    }
}

void GainReductionComputer::computeLinearGainFromSidechainSignal(
    const float* sideChainSignal, float* destination, const int numSamples) {
...
void GainReductionComputer::getCharacteristic(float* inputLevelsInDecibels, float* dest,
