...
void LookAheadGainReduction::setDelayTime(float delayTimeInSeconds) {
    ...
    if (sampleRate != 0.0) prepare(sampleRate, blockSize);
}
...
void LookAheadGainReduction::pushSamples(const float* src, const int numSamples) {
    ...
    for (int i = 0; i < blockSize1; ++i) buffer[startIndex + i] = src[i];

    if (blockSize2 > 0)
        for (int i = 0; i < blockSize2; ++i) buffer[i] = src[blockSize1 + i];
...
void LookAheadGainReduction::process() {
    /** The basic idea here is to look for high gain-reduction values in the signal, and
     apply a fade which starts exactly  `delayInSamples` many samples before that value
     appears. Depending on the value itself, the slope of the fade will vary.

     Some things to note:
        - as the samples are gain-reduction values in decibel, we actually look for
     negative peaks or local minima.
        ...
        - with that slope, we can calculate the next value of our fade-in
     `nextGainReductionValue`
        - once a value in the buffer is below our fade-in value, we found a new minimum,
     which might not be as deep as the previous one, but as it comes in earlier, it needs
     more attention, so we update our fade-in slope
        - our buffer is a ring-buffer which makes things a little bit messy
     */

    // As we don't know any samples of the future, yet, we assume we don't have to apply a
    // fade-in right now, and initialize both `nextGainReductionValue` and step (slope of
    // the fade-in) with zero.
    float nextGainReductionValue = 0.0f;
    float step = 0.0f;

    // Get the position of the last sample in the buffer, which is the sample right before
    // our new write position.
    int index = writePosition - 1;
    if (index < 0)  // in case it's negative...
        index +=
            static_cast<int>(buffer.size());  // ... add the buffersize so we wrap around.

    // == FIRST STEP: Process all recently pushed samples.

    // We want to process all `lastPushedSamples` many samples, so let's find out, how
    // many we can process in a first run before we have to wrap around our `index`
    // variable (ring-buffer).
    int size1, size2;
    ...
    for (int i = 0; i < size1; ++i) {
        ...
        {
            buffer[index] =
                nextGainReductionValue;      // ... replace it with the current ramp value
    ...
    {
        index = static_cast<int>(buffer.size()) -
                1;  // wrap around: start from the last sample of the buffer

        // exactly the same procedure as before... I guess I could have written that
        // better...
    ...
    /*
     At this point, we have processed all the new gain-reduction values. For this, we
     actually don't need a delay/lookahead at all.
     !! However, we are not finished, yet !!
     What if the first pushed sample has such a high gain-reduction value, that itself
     needs a fade-in? So we have to apply a gain-ramp even further into the past. And that
     is exactly the reason why we need lookahead, why we need to buffer our signal for a
     short amount of time: so we can apply that gain ramp for the first handful of
    ...
    /*
     ...
     And there's another cool thing!
        We know that the samples have been processed already, so in case one of the
     samples is below our ramp value, that's the new minimum, which has been faded-in
     already! So what we do is hit the break, and call it a day!
     */
...
void LookAheadGainReduction::readSamples(float* dest, int numSamples) {
    ...
    for (int i = 0; i < blockSize1; ++i) dest[i] = buffer[startIndex + i];

    if (blockSize2 > 0)
        for (int i = 0; i < blockSize2; ++i) dest[blockSize1 + i] = buffer[i];
}

inline void LookAheadGainReduction::getProcessPositions(int startIndex, int numSamples,
                                                        int& blockSize1,
                                                        int& blockSize2) {
    if (numSamples <= 0) {
        blockSize1 = 0;
...
inline void LookAheadGainReduction::getWritePositions(int numSamples, int& startIndex,
...
inline void LookAheadGainReduction::getReadPositions(int numSamples, int& startIndex,
                                                     int& blockSize1, int& blockSize2) {
    const int L = static_cast<int>(buffer.size());
