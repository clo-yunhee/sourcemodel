#ifndef SOURCEMODEL__FZERO_H
#define SOURCEMODEL__FZERO_H
...
template <typename Func>
double fzero(double a, Func f, double tol = std::numeric_limits<double>::epsilon()) {
    ...
    // search
    const double aa = (std::abs(a) < 0.001) ? ((a == 0) ? 0.1 : sgn(a) * 0.1) : a;
